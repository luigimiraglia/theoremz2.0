...
async function main() {
  ...
  console.log(
    `✓ Trovate ${sanityImages.length} immagini referenziate in Sanity\n`
  );

  // 3. Trova le immagini mancanti
  const missingImages = sanityImages.filter(
    (img) => !publicImages.has(img.imageName)
  );
  ...
  if (missingImages.length === 0) {
    console.log(
      "✅ Tutte le immagini referenziate in Sanity sono present in public/images!\n"
    );
  } else {
