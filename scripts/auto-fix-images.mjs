...
function findCorrectUrl(currentUrl, publicImages) {
  ...
  const urlWithoutDomain = currentUrl
    .replace("https://theoremz.com/", "")
    .replace("http://theoremz.com/", "");
  ...
  if (urlMappings.has(currentUrl)) {
    ...
    if (
      publicImages.has(newPath) ||
      publicImages.has(newPath.replace("images/", ""))
    ) {
      return newUrl;
    }
  }
  ...
  if (
    publicImages.has(urlWithoutDomain) ||
    publicImages.has(urlWithoutDomain.replace("images/", ""))
  ) {
    return null;
  }
  ...
  if (urlWithoutDomain.includes("_")) {
    ...
    if (
      publicImages.has(withDash) ||
      publicImages.has(withDash.replace("images/", ""))
    ) {
...
async function main() {
    ...
    for (let i = 0; i < doc.content.length; i++) {
      ...
      if (block._type === "imageExternal" && block.url) {
        ...
        if (correctUrl && correctUrl !== block.url) {
          ...
          !correctUrl &&
          !block.url.startsWith("https://theoremz.com/images/")
        ) {
          notFound++;
        } else {
          skipped++;
  ...
  if (updated > 0) {
    ...
    console.log(
      "💡 Esegui nuovamente check-missing-images.mjs per verificare le immagini ancora mancanti.\n"
    );
  }
}
