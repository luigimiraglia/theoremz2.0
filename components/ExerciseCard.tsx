...
export default function ExerciseCard({ ex }: { ex: Exercise }) {
  ...
  return (
    <motion.article
      ...
      className="group relative overflow-hidden rounded-xl border-4 border-[#2b7fff] bg-white shadow-lg hover:shadow-2xl transition-shadow w-full max-w-6xl mx-auto"
    >
      <div className="h-2 w-full bg-gradient-to-r from-[#2b7fff] via-[#559dff] to-[#2b7fff]" />
      ...
      <div className="p-4">
        <div className="flex-1 min-w-0">
          ...
          {ex.lesson?.title && (
            <Link
              href={`/${ex.lesson.slug}`}
              className="mt-2 inline-flex items-center gap-1 rounded-full border-2 border-[#2b7fff] bg-gradient-to-r from-[#2b7fff]/10 to-[#559dff]/20 px-3 py-1 text-sm font-medium text-[#1a5fd6] hover:from-[#2b7fff]/20 hover:to-[#559dff]/30"
            >
              {ex.lesson.title}
            </Link>
          )}
        ...
        <div className="mt-6 flex flex-wrap gap-4">
          ...
          {ex.passaggi?.length && (
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg bg-gradient-to-r from-[#dce9ff] to-[#a8cfff] px-4 py-1.5 text-sm font-semibold text-[#1a5fd6] ring-2 ring-[#2b7fff] hover:from-[#c7dcff] hover:to-[#90baff]"
            >
        ...
        <AnimatePresence initial={false}>
          {showSol && ex.soluzione?.length && (
            <motion.div
              ...
              className="mt-4 overflow-hidden rounded-lg border-2 border-emerald-400 bg-gradient-to-r from-emerald-50 to-emerald-100 p-4"
            >
        ...
        <AnimatePresence initial={false}>
          {showSteps && ex.passaggi?.length && (
            <motion.div
              ...
              exit={{ height: 0, opacity: 0 }}
              transition={{ duration: 0.25, ease: "easeInOut" }}
              className="mt-4 overflow-hidden rounded-lg border-2 border-[#2b7fff] bg-gradient-to-r from-[#f5f9ff] to-[#dce9ff] p-4"
            >
              <div className="flex items-center gap-2 text-[#1a5fd6] text-sm font-bold">
