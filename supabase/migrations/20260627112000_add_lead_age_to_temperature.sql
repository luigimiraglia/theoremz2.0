...
begin
  ...
  last_seen_age_days := greatest(0, floor(extract(epoch from (now() - coalesce(new.last_seen_at, new.updated_at, new.created_at, now()))) / 86400)::integer);
  ...
  if last_seen_age_days <= 7 and new.last_seen_at > coalesce(new.first_seen_at, new.created_at) + interval '1 hour' then
    score := score + 8;
    reasons := array_append(reasons, 'recent_activity');
  elsif last_seen_age_days > 60 then
    score := score - 6;
    reasons := array_append(reasons, 'stale_activity');
  end if;
...
drop view if exists public.ranked_leads;

create view public.ranked_leads as
select
  l.*,
  greatest(0, floor(extract(epoch from (now() - coalesce(l.first_seen_at, l.created_at))) / 86400)::integer) as lead_age_days,
  greatest(0, floor(extract(epoch from (now() - coalesce(l.last_seen_at, l.updated_at, l.created_at))) / 86400)::integer) as last_seen_age_days,
  row_number() over (
    order by
