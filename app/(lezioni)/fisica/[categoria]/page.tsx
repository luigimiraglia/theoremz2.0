...
const LESSONS_BY_CAT = groq`*[_type=="lesson" && materia=="fisica" && count(categoria[lower(@)==$cat])>0]{ title, slug } | order(title asc)`;

export async function generateStaticParams() {
  // Retrieves all categories for fisica lessons and returns slugs
  const docs = await sanityFetch<{ categoria?: string[] }[]>(
    groq`*[_type=="lesson" && materia=="fisica"]{ categoria }`
  );
...
export default async function Page({ params }: { params: Promise<{ categoria: string }> }) {
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 py-6">
      ...
      <p className="text-slate-600 mt-1">Lezioni correlate in fisica.</p>
