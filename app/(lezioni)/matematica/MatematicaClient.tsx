...
/* Skeleton card (leggero, no animazioni above-the-fold) */
...
export default function MatematicaClient({
  ...
  const [loading] = useState(false); // niente fetch lato client => non “carichiamo”

  // Fuse on-demand
  const fuseRef = useRef<any | null>(null);
  const fuseLoadedRef = useRef(false);

  // Ricerca: import dinamico solo quando si digita
  useEffect(() => {
    const q = query.trim();
    if (!q) {
  ...
  }, [query, selectedClasse, filteredByInterface, groupedByCategory]);
